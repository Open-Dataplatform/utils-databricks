...
from pyspark.sql.types import ArrayType, StructType, StringType, IntegerType, BooleanType, DoubleType, StructField
from pyspark.sql import DataFrame
from custom_utils.config.config import Config
from custom_utils.logging.logger import Logger
from custom_utils.dp_storage import reader  # Ensure reader is correctly imported
...
class Transformer:
    ...
    def json_schema_to_spark_struct(self, schema_file_path, definitions=None):
        """
    ...
    def get_json_depth(self, json_schema, current_depth=0, definitions=None, logger=None, depth_level=None) -> int:
        def calculate_depth(schema, current_depth, definitions):
            ...
            if isinstance(schema, dict):
                if 'properties' in schema:
                    properties_depth = max(calculate_depth(v, current_depth + 1, definitions) for v in schema['properties'].values())
            ...
            if isinstance(schema, list):
                list_depths = [calculate_depth(item, current_depth + 1, definitions) for item in schema]
    ...
    def process_and_flatten_json(self, schema_file_path: str, data_file_path: str, depth_level: int = None, include_schema: bool = False) -> Tuple[DataFrame, DataFrame]:
        ...
        try:
            schema_json, schema = self.json_schema_to_spark_struct(schema_file_path)
            depth_level_to_use = depth_level if depth_level is not None else 1
            max_depth = self.get_json_depth(schema_json, logger=None, depth_level=depth_level_to_use)
